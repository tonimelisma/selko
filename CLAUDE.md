...
- **Pending-change integrity:** An event may remain `pending_change` only while it has an active, non-undone update or cancellation source; a deferred database constraint enforces that invariant. Accept and reject transitions use the service-only `apply_pending_event_change` and `reject_pending_event_change` RPCs so the event and its proposal change atomically. Review clients fail closed when proposal details are unavailable.
