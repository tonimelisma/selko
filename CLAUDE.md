...
- **Two transports, no fallback between them.** RLS-scoped client and API traffic uses PostgREST with the *user's* JWT. Trusted-worker coordination (claim/heartbeat/complete/lease) uses asyncpg over the Supavisor **session pooler, port 5432** — PostgREST costs ~1,690 B/call of pure envelope for a service-role caller that bypasses RLS anyway. **An operation has exactly one implementation.** Never write `if pg_pool is not None: ... else: ...`; never add a `_via_pool` twin. The worker port is in progress — see `docs/specs/direct-pg-completion-and-live-ui-hardening.md` C2 for the inventory and the guard test that enforces this.
