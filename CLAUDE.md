...
- **Durable email polling v2:** when `ENABLE_EMAIL_INGESTION_V2=true`, `IngestionRuntime` starts inside the FastAPI process (async monolith — no separate Render service) and owns leased provider discovery, durable identity acquisition, independent attachment retries, reconciliation, and safe health notifications. Single ownership comes from database leases (`FOR UPDATE SKIP LOCKED` + lease expiry), not from process topology. APScheduler stays off in this mode; the old `email_fetch` path is rollback-only. `selko.worker_app` runs the identical task set standalone for staging drills.
