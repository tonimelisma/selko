...
from selko.services.gmail import fetch_messages
...
class TestFetchMessages:
    """Tests for fetch_messages label filtering."""

    def test_default_omits_label_filter(self):
        """Default fetch should not restrict to INBOX (archived mail still pulled)."""
        service, list_mock = _mock_gmail_service()

        fetch_messages(service, max_results=10)

        list_mock.assert_called_once_with(userId="me", maxResults=10)
