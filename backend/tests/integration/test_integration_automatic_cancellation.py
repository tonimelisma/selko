...
class TestAutomaticCancellation:
    ...
    async def test_cancel_claim_and_completion_are_worker_owned(
        ...
        final = admin_client.table("events").select(
            "review_status,title,google_calendar_event_id"
    ...
    async def test_stale_upsert_completion_cannot_reset_cancellation(
        ...
        row = admin_client.table("events").select("review_status").eq(
            "id", event_id
        ).single().execute().data
        assert row["review_status"] == "active"
        work_item = admin_client.table("calendar_work_items").select(
            "status,action"
        ).eq("event_id", event_id).order("generation", desc=True).limit(1).single().execute().data
        assert work_item == {"status": "pending", "action": "cancel"}
