...
class TestAdoptingNewLaneItemsTheCalendarAlreadyHolds:
    """A New-lane item whose join link is on the user's calendar is not new.

    The rule deliberately keys on the join link, never on start time: production
    holds unrelated pairs that collide on start time alone (a teacher placement
    release and a pub get-together), so resolving by time would mark unrelated
    events as already-handled.
    """

    class _Hint:
        def __init__(self, kind, value_hash):
            self.kind = kind
            self.value_hash = value_hash

    def _client(self, hint_rows, entry_rows):
        client = MagicMock()
        hints_q = MagicMock()
        hints_q.execute.return_value = MagicMock(data=hint_rows)
        client.table.return_value.select.return_value.eq.return_value.eq.return_value.in_.return_value.not_.is_.return_value = hints_q
        entry_q = MagicMock()
        entry_q.execute.return_value = MagicMock(data=entry_rows)
        client.table.return_value.select.return_value.eq.return_value.eq.return_value.is_.return_value = entry_q
        return client

    def test_a_shared_join_link_names_the_entry_to_adopt(self):
        from selko.services.events import _calendar_entry_sharing_a_join_link

        client = self._client(
            [{"calendar_entry_id": "entry-1"}], [{"provider_event_id": "gcal-1"}]
        )
        assert _calendar_entry_sharing_a_join_link(
            client, "u1", [self._Hint("join_url", "hash-1")]
        ) == "gcal-1"

    def test_an_event_with_no_join_hint_is_never_adopted(self):
        """Most events have only a UID hint or none; they must stay in New."""
        from selko.services.events import _calendar_entry_sharing_a_join_link

        client = self._client([{"calendar_entry_id": "entry-1"}], [{"provider_event_id": "g"}])
        assert _calendar_entry_sharing_a_join_link(
            client, "u1", [self._Hint("ical_uid", "hash-1")]
        ) is None

    def test_a_link_shared_by_two_entries_names_no_single_event(self):
        """A recurring standing link must not silently adopt one occurrence."""
        from selko.services.events import _calendar_entry_sharing_a_join_link

        client = self._client(
            [{"calendar_entry_id": "entry-1"}, {"calendar_entry_id": "entry-2"}],
    ...
    def test_a_failed_lookup_leaves_the_item_in_the_new_lane(self):
        from selko.services.events import _calendar_entry_sharing_a_join_link
