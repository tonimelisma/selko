-- Close the attachment-descriptor race (top-up increment 4).
--
-- `acquire_item` used to upsert the email row (processing_status defaulting to
-- 'pending') *first*, then issue a SELECT + INSERT per attachment descriptor in
-- separate PostgREST round-trips. Between the email upsert and the first
-- descriptor insert the readiness gate in `claim_unprocessed_email` saw zero
-- attachment rows, so the gate passed — and an LLM worker could claim the
-- email before its attachment rows existed. With three pool workers polling
-- every 1s the window for a five-attachment email was ten sequential round
-- trips wide. The LLM then processed the body with no attachments, which
-- presented as flaky extraction quality rather than as a race.
--
-- Fix: one RPC commits the email upsert AND its attachment descriptors in a
-- single transaction, so the readiness gate is never observed in the gap.
...
DECLARE
    ...
    -- Only the payload-driven columns of public.emails are written, so
    -- processing/lock/attempt columns (processing_status, locked_by,
    -- attempts, next_retry_at, ...) are preserved exactly as the PostgREST
    -- upsert preserved them. Missing payload keys keep the column default
    -- on INSERT (e.g. processing_status stays 'pending', not NULL).
...
BEGIN
    ...
    -- Insert any descriptor not already present for this email_id. The partial
    -- unique index attachments_email_provider_attachment_idx matches the ON
    -- CONFLICT predicate exactly. ON CONFLICT DO NOTHING preserves a previously
    -- stored row — re-acquisition must never reset ingestion_status='stored'.
...
COMMENT ON FUNCTION public.save_email_with_attachment_descriptors(uuid, jsonb, jsonb) IS
    'Atomically upsert an email row and its pending attachment descriptors so claim_unprocessed_email never sees an email whose descriptors have not been written yet.';