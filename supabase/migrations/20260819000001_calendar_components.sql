-- C1: provider-neutral calendar components plus the single four-argument
-- acquisition write path. Components are opaque correlation data and remain
-- service-role-only.
...
CREATE TABLE public.email_calendar_components (
    ...
    uid_hash text,
    recurrence_id text,
    recurrence_range text CHECK (recurrence_range IS NULL OR recurrence_range = 'THISANDFUTURE'),
...
BEGIN
    ...
    SELECT p_user_id, v_email_id, d->>'provider_attachment_id',
        ...
        COALESCE(NULLIF(d->>'size_bytes', '')::bigint, NULLIF(d->>'size', '')::bigint, 0), 'pending'
    ...
    ON CONFLICT (email_id, provider_attachment_id)
        WHERE provider_attachment_id IS NOT NULL AND provider_attachment_id <> '' DO NOTHING;

    -- Empty means the provider did not expose calendar data on this save. It
    -- must not erase components obtained by an earlier full fetch.
    IF jsonb_array_length(COALESCE(p_calendar_components, '[]'::jsonb)) > 0 THEN
        ...
        INSERT INTO public.email_calendar_components
            (email_id, user_id, component_index, method, uid_hash, recurrence_id,
             recurrence_range, sequence, dtstamp, component_status, start_datetime, end_datetime)
        SELECT v_email_id, p_user_id, (ord - 1)::integer,
            c->>'method', c->>'uid_hash', c->>'recurrence_id', c->>'recurrence_range',
            COALESCE(NULLIF(c->>'sequence', '')::integer, 0),
            NULLIF(c->>'dtstamp', '')::timestamptz, c->>'component_status',
            NULLIF(c->>'start_datetime', '')::timestamptz, NULLIF(c->>'end_datetime', '')::timestamptz
        FROM jsonb_array_elements(p_calendar_components) WITH ORDINALITY AS t(c, ord);
    END IF;
    RETURN v_email_id;
END; $$;
...
COMMENT ON TABLE public.email_calendar_components IS 'Provider-neutral calendar components per email (C1)';
