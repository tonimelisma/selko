-- Fix forward: the health rollup must measure scheduled polls only for active
-- integrations. Expired integrations are intentionally not claimable, so an
-- old next_poll_at on one must not make the whole service report degraded.
-- This is the same scope already used by integrations_due below.
