<script>
	...
	let eventStatusMap = $derived(/** @type {Record<string, {class: string, label: string}>} */ ({
		pending_review: { class: 'badge-changed', label: $_('status.pending') },
		pending_change: { class: 'badge-changed', label: $_('status.pendingChange') },
		approved: { class: 'badge-neutral-warm', label: $_('status.approved') },
		syncing: { class: 'badge-neutral-warm', label: $_('status.syncing') },
		synced: { class: 'badge-success', label: $_('status.synced') },
		sync_failed: { class: 'badge-error', label: $_('status.failed') },
		rejected: { class: 'badge-ghost badge-neutral-warm', label: $_('status.rejected') },
		cancelled: { class: 'badge-ghost badge-neutral-warm', label: $_('status.cancelled') }
	}));
	...
	let integrationStatusMap = $derived(/** @type {Record<string, {class: string, label: string}>} */ ({
		...
		not_connected: { class: 'badge-ghost badge-neutral-warm', label: $_('status.notConnected') }
	}));
